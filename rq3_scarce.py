...
# ── Scarce-data sweep settings ─────────────────────────────────────────────
# Each size runs the FULL rq3 experiment end-to-end:
#   subsample → learn BIC structure → Tabular CPD + DeepCPD
#   subsample → learn mBIC structure → Tabular CPD + DeepCPD
...
def subsample_stratified(df_int, size, target_node, seed):
    """Stratified subsample preserving target class balance."""
    try:
...
def run_rq3_scarce(output_dir="RQ3_SCARCE_RESULTS"):
    ...
    def decode_to_str(df_int):
        """Convert integer-encoded frame back to string for structure learning."""
        out = df_int.copy()
    ...
    test_int  = encode(test_str)   # fixed — never subsampled
    ...
    # ── reference structures from full data (for SHD reporting only) ─────────
    ...
    for size in SUBSAMPLE_SIZES:
        ...
        # collect per-seed results before averaging
        seed_records = {
        ...
        for seed in SWEEP_SEEDS:
            ...
            # subsample ONCE per seed — same subset used for BIC and mBIC
            ...
            # learn both structures on THIS subset
            structures = {}
            for score_name in ["BIC", "mBIC"]:
                set_seed(seed)
                try:
            ...
            # 2x2: for each structure, fit Tabular then DeepCPD
            for score_name in ["BIC", "mBIC"]:
        ...
        # ── aggregate across seeds and write records ──────────────────────────
    ...
    # ── crossover plots (one per structure) ───────────────────────────────────
    try:
